...
class data2doc():
    ...
    def __init__(self, dataDic={}, docxTemplateBinary="",
                 xlsxBinary="", jsonBinary=""):
    ...
    def setNmFmt(self, cellText, formatStr):
        ...
        if re.match(r"0\.0*E\+00", formatStr):  # scientific
            ...
            cellText = datetime.fromordinal(datetime(1900, 1, 1).toordinal() +
                                            int(N(cellText)) - 2).strftime("%m/%d/%Y")
    ...
    def data2doc(self):
        ...
        for tableName in self.dataDic:
            ...
            for y in range(len(docxRowXml)):
                ...
                startRow, columnNamesRow, endRow, filterRow, rowEval, columnNamesProxy =\
                ...
                for rowCount in range(1, max(self.dataDic[tableName]) + 1):
                    ...
                    for columnName in row:
                        ...
                        tmpDocxXml = tmpDocxXml.replace("#%s#" %
                                                        columnNamesProxy.get(columnName, columnName),
                                                        row[columnName])
        ...
        while "#@#" in dxDoc[startPoint:]:
            ...
            if parStart > 0 and parEnd > 0:
                ...
                parText = ("".join([x.text
                           for x in
                           gg.findall("./{ww}p/{ww}r/{ww}t")])).replace("#@#", "").strip()
                if parText == "":
    ...
    def setTestData(self):
        self.dataDic = {'header': 'Headet Text Example',
                        'data': [{'id': "1", 'name': "john", "address": "new york"},
