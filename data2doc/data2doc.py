...
class data2doc():
	...
	def xlsxBinary2dataDic(self):
		...
		for x in xlsxZip.namelist():
			...
			#remove empty cells
			sheetText="".join([y for y in re.split(r"(<[^<]+>)",sheetText) \
	...
	def setNmFmt(self, cellText, formatStr):
		...
		# elif re.match(r'd". "mmm". "yyyy',formatStr):# 9. Jan. 1955
		# 	cellText=datetime.fromordinal(datetime(1900, 1, 1).toordinal() + int(N(cellText)) - 2).strftime("%d. %b. %Y")
			...
			cellText=datetime.fromordinal(datetime(1900, 1, 1).toordinal() + int(N(cellText)) - 2).strftime("%m/%d/%Y")
	...
	def data2doc(self):
		...
		for tableName in self.dataDic:
			...
			for y in range(len(docxRowXml)):
				...
				startRow, columnNamesRow, endRow, filterRow, rowEval, columnNamesProxy = self.getTableParams(docxRowXml[y], tableName)
				for rowCount in range(1,max(self.dataDic[tableName])+1):
					...
					for columnName in row:
						...
						tmpDocxXml=tmpDocxXml.replace("#%s#" % columnNamesProxy.get(columnName,columnName),row[columnName])
		...
		while "#@#" in dxDoc[startPoint:]:
			startPoint=parEnd=parStart=tableTagStart=dxDoc.index("#@#",startPoint)
			while parStart>0:
			...
			if parStart>0 and parEnd>0:
				...
				xmlns="".join(["""xmlns:%s="%s%s" """ % (x,x,x) 
							for x in { x.translate("".maketrans("","","</: "))
								for x in re.findall(r"\W(\w*):",parXml) if x!="xml"} ])
				gg=ET.fromstring(f"""<r {xmlns}>{parXml}</r>""")
				parText=("".join([x.text for x in gg.findall("./{ww}p/{ww}r/{ww}t")])).replace("#@#","").strip()
				if parText=="":
	...
	def getSnippetRow(self,xml,tableName):
		...
		while tag in xml:
			...
			tableProps=xml[tag1_pos+1:xml[tag1_pos+1:].index("#")+tag1_pos+1].replace(tableName,"")
			if tag in xml[tag1_pos+1:]:
				tag2_pos=xml.index(tag,len(tag)+tag1_pos)+len(tag)+1
				snippet=xml[tag1_pos:tag2_pos]
				if "<w:tbl>" in snippet and not "</w:tbl>" in snippet:
					snippet=""
			if snippet:
	...
	def setTestData(self):
		self.dataDic={'header':'Headet Text Example',
				'data':[
...
if __name__ == '__main__':
	...
	if merge(f"{testSourceFolder}test.docx",
